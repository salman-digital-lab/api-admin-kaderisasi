...
class UserController {
  ...
  async store({ request, response }) {
    ...
    const { username, email, password, first_name, last_name, group_id } =
      request.all();
    ...
    try {
      ...
      return response.status(500).json({ status: "FAILED", message: err.code });
    }
  }
  ...
  async update({ request, response, params }) {
    ...
    try {
      await User.query(trx).where("id", id).update(
        {
          username,
          display_name,
          email,
      ...
      await UsersGroup.query(trx).where("user_id", id).update({ group_id });
