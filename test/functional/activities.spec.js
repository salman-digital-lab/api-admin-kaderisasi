...
function assertResponseIsSameWithActivity(responseObject, sampleActivity) {
  sampleActivity.activityCategory.created_at =
    sampleActivity.activityCategory.created_at.toJSON();
    ...
    data: [
      {
        ...
        maximum_role_id: sampleActivity.maximum_role_id,
        activityCategory: sampleActivity.activityCategory,
        minimumRole: sampleActivity.minimumRole,
        maximumRole: sampleActivity.maximumRole,
        carousel: sampleActivity.carousel,
      },
    ],
  });
}
...
test("get activity detail", async ({
  getUser,
  getAnActivity,
  assert,
  client,
  ...
  assertResponseIsSameWithActivity(response, sampleActivity.toJSON());
});

test("get non-existent activity detail", async ({
  assert,
  client,
  getUser,
}) => {
  ...
  const response = await client.get(`v1/activity/12222`).loginVia(user).end();
...
test("get deleted activity detail", async ({
  assert,
  client,
  getAnActivity,
  getUser,
...
test("update with same event begin and end date", async ({
  assert,
  client,
  getAnActivity,
  getUser,
...
test("get activity participants", async ({
  assert,
  client,
  getAnActivity,
  getUser,
...
test("participants of non-existent activity", async ({
  assert,
  client,
  getAnActivity,
  getUser,
...
// Using random participation data
test("test invalid participation status change", async ({
  assert,
  client,
  getUser,
}) => {
...
test("test for data export", async ({
  assert,
  client,
  getUser,
  getAnActivity,
...
test("test for participant statistics", async ({
  assert,
  client,
  getUser,
  getAnActivity,
...
test("test fetch questionnaire - normal flow", async ({
  assert,
  client,
  getUser,
  getAnActivity,
...
test("test fetch questionnaire - missing keys from answer", async ({
  assert,
  client,
  getUser,
  getAnActivity,
