...
Factory.blueprint("App/Models/Activity", async (faker, index, data) => {
  ...
  if (maximum_role_id.index < minimum_role_id.index)
    maximum_role_id.id = minimum_role_id.id;
  ...
  return {
    ...
    register_end_date: faker.date({
      year: 2021,
      month: faker.integer({ min: 1, max: 10 }),
    }),
...
Factory.blueprint("App/Models/Komprof", (faker) => {
  return {
    program_name: faker.name(),
    program_desc: faker.text,
    is_active: 1,
  };
});
